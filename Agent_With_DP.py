...
import os, re, torch, json, glob
from transformers import GPT2Tokenizer, GPT2LMHeadModel, StoppingCriteria, StoppingCriteriaList
from peft import PeftModel


def find_latest_lora_model():
    patterns = [
        "/content/runs/dp_lora_*/dp_lora_model",
        "runs/dp_lora_*/dp_lora_model"             
    ]
    for pattern in patterns:
        matches = glob.glob(pattern)
        if matches:
            latest = sorted(matches)[-1]
            print(f"✓ Auto-detected LoRA model: {latest}")
            return latest
    return None

LORA_DIR = find_latest_lora_model() or "runs/dp_lora_20251103_153331/dp_lora_model"
...
if LORA_DIR and os.path.exists(LORA_DIR):
    ...
    print(f"⚠ LoRA model not found at: {LORA_DIR}")
    print("⚠ Using base GPT-2 model without LoRA fine-tuning")
    model = base
...
example_dialogue = """Patient John Doe (555-123-4567) presents with cough and fever for 3 days.
